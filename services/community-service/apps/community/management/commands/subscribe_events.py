"""Redis Pub/Sub event subscriber daemon for community-service."""
import json
import logging
import signal
import sys
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('community-service event subscriber started.')

        def shutdown(sig, frame):
            self.stdout.write('Shutting down subscriber…')
            pubsub.unsubscribe()
            pubsub.close()
            sys.exit(0)
